...
class ProbeServer {
  private readonly localObservations: ProbeObservation[] = [];
  private readonly logPath = `/tmp/aegis-ts-broker-probe-${randomUUID()}.jsonl`;
  private wslProbe?: ChildProcess;
  ...
  get observations(): ProbeObservation[] {
    if (this.wslProbe) return readWslProbeObservations(this.logPath);
    return [...this.localObservations];
  }

  async start(): Promise<number> {
    if (process.platform === "win32") return this.startViaWsl();
    await new Promise<void>((resolve) => this.server.listen(0, "127.0.0.1", () => resolve()));
    const address = this.server.address();
    if (!address || typeof address === "string") throw new Error("probe server failed to bind");
    return address.port;
  }
  ...
  async stop(): Promise<void> {
    if (this.wslProbe) {
      this.wslProbe.kill();
      await new Promise<void>((resolve) => this.wslProbe?.once("exit", () => resolve()));
  ...
  private async startViaWsl(): Promise<number> {
...
export async function runAllowedCase(): Promise<void> {
  requireSecret();
...
export async function runDeniedCase(): Promise<void> {
  requireSecret();
  const client = new AegisClient();
  await assertHealth(client);
  const probe = new ProbeServer();
...
async function assertHealth(client: AegisClient): Promise<void> {
  const health = await client.health();
  if (!health.ok) throw new Error(`Aegis health check failed: status=${health.status}`);
}
...
function secretFromOrchestratorEnv(): string {
  ...
  for (const entry of readdirSync(procRoot)) {
    ...
    try {
      ...
      if (!cmdline.includes(".aegis/bin/orchestrator") && !cmdline.includes("/tmp/aegis-bin")) continue;
...
function assertStdoutMarker(stdout: string, marker: string): void {
  if (!stdout.includes(marker)) throw new Error(`expected stdout marker ${marker}, got: ${stdout}`);
}

async function assertResultArtifacts(result: { ok: boolean; throwIfExecutionFailed(): void; proofDir?: string; receiptPath?: string; receipt: { verify(): Promise<{ verified: boolean }> } | undefined }): Promise<void> {
  if (!result.ok) result.throwIfExecutionFailed();
  if (!result.proofDir || !existsSync(result.proofDir)) throw new Error("missing proofDir on disk");
  if (!result.receiptPath || !existsSync(result.receiptPath)) throw new Error("missing receiptPath on disk");
  if (!result.receipt) throw new Error("result.receipt could not be loaded");
  const verification = await result.receipt.verify();
  if (!verification.verified) throw new Error("receipt verification did not return verified");
}

function assertProbeRequests(observations: ProbeObservation[], options: { expected: number; requireAuth: boolean }): void {
  if (observations.length !== options.expected) throw new Error(`expected ${options.expected} upstream requests, got ${observations.length}`);
...
async function assertNoSecretLeak(result: { stdout: string; stderr: string; proofDir?: string }): Promise<void> {
  const secret = requireSecret();
  const leaks: string[] = [];
  if (result.stdout.includes(secret)) leaks.push("stdout");
  if (result.stderr.includes(secret)) leaks.push("stderr");
  if (result.proofDir) {
    for (const path of await scanDirectoryForSecret(result.proofDir, secret)) leaks.push(path);
  }
  if (leaks.length > 0) throw new Error(`raw credential leak detected in: ${leaks.join(", ")}`);
}

async function scanDirectoryForSecret(root: string, secret: string): Promise<string[]> {
  ...
  async function walk(current: string): Promise<void> {
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (data.includes(Buffer.from(secret, "utf8"))) findings.push(path);
      }
    }
  }
...
function brokerEvidenceStatus(input: { executionId: string; receiptSummaryPath?: string; expected: "allowed" | "denied" }): string {
  ...
  if (input.expected === "denied" && input.receiptSummaryPath && existsSync(input.receiptSummaryPath)) {
    ...
    if (summary.includes("broker.request_denied")) return "receipt:broker.request_denied";
  }
  return input.expected === "allowed" ? "upstream_probe_auth_present" : "receipt:broker.request_denied";
}

function telemetryEventsPresent(executionId: string, expectedKinds: string[]): boolean {
...
function brokerIntent(input: { executionId: string; allowedDomains: string[]; allowedDelegations: string[]; taskClass: string; declaredPurpose: string }): IntentContract {
...
function allowedGuestCode(port: number): string {
...
function deniedGuestCode(port: number): string {
  return `#!/usr/bin/env bash
set -euo pipefail
exec 3<>/dev/tcp/127.0.0.1/8888
printf 'GET http://127.0.0.1:${port}/probe HTTP/1.1\r\nHost: 127.0.0.1:${port}\r\nConnection: close\r\n\r\n' >&3
...
function buildWslProbeCommand(logPath: string): string {
  const pythonCode = [
...
function readWslProbeObservations(logPath: string): ProbeObservation[] {
  const uncPath = toWslUncPath(logPath);
  if (!existsSync(uncPath)) return [];
...
function toWslUncPath(path: string): string {
