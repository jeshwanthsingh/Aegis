...
export class ReceiptVerifier {
    ...
    resolveCliPath() {
        const candidates = [];
        if (this.cliPath)
            candidates.push(this.cliPath);
        if (process.env.AEGIS_CLI_BIN)
            candidates.push(process.env.AEGIS_CLI_BIN);
    ...
    async verifyReceipt(options) {
        ...
        if (options.receiptPath) {
            ...
            if (options.publicKeyPath)
                command.push("--public-key", options.publicKeyPath);
        }
...
function buildInvocation(cli, args) {
...
function shouldInvokeViaWsl(cli) {
...
function isMappedWslDrivePath(path) {
