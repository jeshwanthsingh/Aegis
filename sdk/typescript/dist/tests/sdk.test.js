...
import { AegisAuthError, AegisClient, AegisConfigurationError, AegisExecutionError, AegisValidationError, BrokerCapabilities, BrokerDelegation, BrokerScope, Budgets, CapabilitiesRequest, DoneEvent, ExecutionRequest, IntentContract, NetworkScope, ProcessScope, ProofEvent, Receipt, ReceiptVerification, ReceiptVerifier, ResourceScope, StdoutEvent, } from "@aegis/sdk";
class TestServer {
    routes;
    requests = [];
    server;
...
test("receipt wrapper and verifier helpers work", async () => {
    ...
    const receipt = Receipt.load(receiptPath, { verifier: new StubVerifier() });
