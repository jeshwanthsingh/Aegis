...
func TestBroker_DomainDenied(t *testing.T) {
	...
	if !resp.Denied { t.Fatal("expected Denied=true") }
	if resp.DenyReason != "broker.domain_denied" { t.Fatalf("wrong reason: %s", resp.DenyReason) }
...
func TestBroker_AllowedDomain_NoCredential(t *testing.T) {
	...
	if resp.Denied { t.Fatalf("expected allowed, denied: %s", resp.DenyReason) }
	if !resp.Allowed { t.Fatal("expected Allowed=true") }
...
func TestBroker_AllowedDomain_WithCredential(t *testing.T) {
	...
	if !resp.Allowed { t.Fatalf("expected allowed, denied: %s", resp.DenyReason) }
	respJSON, _ := json.Marshal(resp)
	if strings.Contains(string(respJSON), "secret-test-value") { t.Fatal("raw credential in response") }
	if receivedAuth != "Bearer secret-test-value" { t.Fatalf("missing auth header: %q", receivedAuth) }
}
...
func TestBroker_ConnectDenied(t *testing.T) {
	...
	if !resp.Denied { t.Fatal("expected CONNECT denied") }
}
...
func TestBroker_NoBrokerScope_DeniesAll(t *testing.T) {
	...
	if !resp.Denied { t.Fatal("expected deny with no domains") }
}
...
func TestBroker_EmitsTelemetryEvents(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Kind == telemetry.KindCredentialRequest { foundReq = true }
		if ev.Kind == telemetry.KindCredentialAllowed { foundAllowed = true }
	}
	if !foundReq { t.Error("credential.request not emitted") }
	if !foundAllowed { t.Error("credential.allowed not emitted") }
}
...
func TestBroker_DenialEmitsTelemetry(t *testing.T) {
	...
	events := bus.Drain()
	foundDenied := false
	for _, ev := range events { if ev.Kind == telemetry.KindCredentialDenied { foundDenied = true } }
	if !foundDenied { t.Error("credential.denied not emitted") }
}
...
func TestLoadBinding_EnvVar(t *testing.T) {
	...
	if !ok { t.Fatal("expected binding loaded") }
	if binding.BearerToken() != "Bearer ghp_testtoken123" { t.Fatalf("wrong token: %s", binding.BearerToken()) }
}

func TestLoadBinding_Missing(t *testing.T) {
	binding, ok := LoadBinding("nonexistent-credential")
	if ok { t.Fatal("expected missing binding") }
	if binding.IsLoaded() { t.Fatal("expected IsLoaded=false") }
}
