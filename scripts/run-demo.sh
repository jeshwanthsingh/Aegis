...
RESULT=$(curl -s -X POST $BASE \
  -H "Content-Type: application/json" \
  -d '{"lang":"python","code":"import urllib.request\nurllib.request.urlopen(\"http://example.com\")","timeout_ms":8000}')
echo $RESULT | jq .
...
RESULT=$(curl -s -X POST $BASE \
  -H "Content-Type: application/json" \
  -d '{"lang":"python","code":"print(open(\"/etc/passwd\").read())","timeout_ms":5000}')
echo $RESULT | jq .
...
for i in 1 2 3 4 5; do
  curl -s -X POST $BASE \
    -H "Content-Type: application/json" \
    -d "{\"lang\":\"python\",\"code\":\"print('worker $i')\",\"timeout_ms\":5000}" \
